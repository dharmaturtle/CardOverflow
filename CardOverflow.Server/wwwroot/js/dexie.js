...
function getDb() {
    ...
    db.version(1).stores({
        Deck: "commandId,clientCreatedAt,serverCreatedAt",
    });
    return db;
}
...
function bulkPutEvents(table, eventsJson) {
    let events = JSON.parse(eventsJson).map(event => {
        return {
            commandId      : event.Fields[0].Meta.CommandId,
            clientCreatedAt: event.Fields[0].Meta.ClientCreatedAt,
            serverCreatedAt: event.Fields[0].Meta.ServerCreatedAt?.Fields[0] ?? unsynced,
