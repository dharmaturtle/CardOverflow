...
let ``SanitizeTagRepository AddTo/DeleteFrom works``(): Task<unit> = (taskResult {
    ...
    let! _ = FacetRepositoryTests.addBasicConcept c.Db otherUser [tagName] (concept_2, example_2, revision_2, [card_2])
...
let ``Tag counts work``(): Task<unit> = (taskResult {
    ...
    let tagName = Guid.NewGuid().ToString() |> MappingTools.toTitleCase
    let! _ = FacetRepositoryTests.addBasicConcept c.Db author [tagName] (concept_1, example_1, revision_1, [card_1])
    do! assertTagUserCount 1

    // initial tag has 2 users after collecting
    do! ConceptRepository.CollectCard c.Db collector revisionId [ Ulid.create ]
...
let ``TagRepository.getAll works``(): Task<unit> = (taskResult {
    use c = new TestContainer()
    let userId = user_3
    let! _ = FacetRepositoryTests.addBasicConcept c.Db userId ["ax" +/+ "by" +/+ "cz"] (concept_1, example_1, revision_1, [card_1])
