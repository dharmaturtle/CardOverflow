...
let ``CardRepository.deleteAcquiredCard works``(): Task<unit> = task {
    ...
    do! CardRepository.deleteAcquiredCard c.Db userId ac.StackId
    ...
    do! CardRepository.deleteAcquiredCard c.Db userId ac.StackId // can delete after adding a history, tag, and relationship
    Assert.Null x.Value
    ...
    //do! reacquire ()
    //let otherUserId = 2
    //do! CardRepository.deleteAcquiredCard c.Db otherUserId ac.StackId
...
let ``CardRepository.editState works``(): Task<unit> = task {
    ...
    Assert.Null x.Value
    let! ac = CardRepository.GetAcquired c.Db userId ac.StackId
    let ac = ac.Value.Single()
