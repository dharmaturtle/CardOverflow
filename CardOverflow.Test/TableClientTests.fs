...
[<StandardProperty>]
let ``can insert, get, and update stack snapshot`` (stack: Stack.Events.Snapshotted, leafId: LeafId) =
    ...
    // update
    tc.Update(fun (x: Stack.Events.Snapshotted) -> { x with CopySourceLeafId = Some leafId }) stack.Id
