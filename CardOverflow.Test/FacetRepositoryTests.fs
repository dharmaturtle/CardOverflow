...
let add gromplateName createCommand (db: CardOverflowDb) userId tags (ids: Guid * Guid * Guid * Guid list) = task {
    ...
    let (_, _, _, cardIds) = ids
    let! r =
        createCommand gromplate (tagIds |> List.ofSeq) ids
        |> SanitizeStackRepository.Update db userId [] cardIds
    return r.Value
    }
...
let update (c: TestContainer) authorId kind commandTransformer updateIds expectedBranchId = taskResult {
    ...
    return!
        upsert
        |> commandTransformer
        |> SanitizeStackRepository.Update c.Db authorId [] [ Ulid.create ]
...
let ``ExploreStackRepository.leaf works``() : Task<unit> = (taskResult {
    ...
    let! actualBranchId = SanitizeStackRepository.Update c.Db userId [] [ Ulid.create ] updated
    ...
    do! SanitizeStackRepository.Update c.Db userId [] [ Ulid.create ] stackCommand
...
let ``ExploreStackRepository.branch works``() : Task<unit> = (taskResult {
    ...
    let newLeafId = leaf_2
    let newValue = Guid.NewGuid().ToString()
    let! old = SanitizeStackRepository.getUpsert c.Db (VUpdate_BranchId branchId) ((stack_1, branch_1, newLeafId, [Ulid.create]) |> UpsertIds.fromTuple)
    let updated = {
        old with
            ViewEditStackCommand.FieldValues =
                old.FieldValues.Select(fun x ->
...
let ``UpdateRepository.card edit/copy/branch works``() : Task<unit> = task {
    ...
    let! actualBranchId = SanitizeStackRepository.Update c.Db user_1 [] [ Ulid.create ] updated |> TaskResult.getOk
    ...
    let! actualBranchId = SanitizeStackRepository.Update c.Db user_2 [] [ Ulid.create ] updated |> TaskResult.getOk
